...
Create `<ISSUE-KEY>/attachments/` in the current working directory (e.g. `PROJ-123/attachments/`) — namespacing by issue key avoids collisions if this runs for multiple tickets in the same repo checkout.
...
Download every attachment this way before moving on.

## 4. Auto-extract archives

For every downloaded file ending in `.zip`, `.tar`, `.tar.gz`, or `.tgz`: extract it into a subfolder named after the archive (minus extension) under `attachments/`, then strip any `__MACOSX` directory the extraction produced. Leave non-archive attachments as-is.

## 5. Write context.md

Write `<ISSUE-KEY>/context.md` with:
...
- **Notes** — anything non-obvious a future reader would otherwise have to rediscover: description is empty and the real spec lives in an attachment, comments contradict the description, an attachment looks stale/superseded by a newer one, a Figma link 404s or looks private, etc. Skip this section only if there's genuinely nothing non-obvious.

Report the folder path to the user when done. Don't summarize the ticket further in chat beyond that — the point of `context.md` is that it's the summary.
